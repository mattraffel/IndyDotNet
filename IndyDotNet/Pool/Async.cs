...
using static IndyDotNet.Pool.NativeMethods;
...
namespace IndyDotNet.Pool
{
    ...
    internal sealed class PoolAsync : IDisposable
    {
        ...
        private static void OpenPoolLedgerCallbackMethod(int command_handle, int err, IntPtr pool_handle)
        {
            ...
            taskCompletionSource.SetResult(new PoolAsync(pool_handle));
        }
        ...
        public static Task<PoolAsync> OpenPoolLedgerAsync(string configName, string config)
        {
            ParamGuard.NotNullOrWhiteSpace(configName, "configName");

            var taskCompletionSource = new TaskCompletionSource<PoolAsync>();
            var commandHandle = PendingCommands.Add(taskCompletionSource);
